...
public class Minimax {
    ...
    public Result search(Board rootBoard, int maxDepth, long timeLimitMs) {
        ...
        // Use full allotted time; soft start-new-depth cutoff, hard abort mid-search.
        // Hard is slightly under the official cap so we always return a move.
        long budgetMs = Math.max(400, timeLimitMs);
        long start = System.nanoTime();
        softDeadlineNanos = start + (long) (budgetMs * 0.90) * 1_000_000L;
        hardDeadlineNanos = start + (long) (budgetMs * 0.97) * 1_000_000L;
