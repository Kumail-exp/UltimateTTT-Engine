...
public class Minimax {
    ...
    private int rootMoveScore(Board board, Move m, int complexityBias) {
        ...
        if (complexityBias > 0) {
            // Prefer moves that leave opponent with more options (harder forced wins)
            // Approximate without full make: center / non-finishing boards
    ...
    /**
     * Stronger evaluation:
     *  - Weighted local board ownership
     *  - Local two-in-a-row threats
     *  - Global (meta) two-in-a-row threats on the big board
     *  - Mobility / open-board pressure
     */
    private int evaluate(Board board, Player perspective) {
        int score = 0;
        int myGlobal = 0, oppGlobal = 0, openMask = 0;

        for (int g = 0; g < 9; g++) {
            GameState ls = board.getLocalState(g);
            int w = GLOBAL_WEIGHT[g];
            if (ls == GameState.WON_X) {
                score += (perspective == Player.X ? 150 : -150) * w;
                if (perspective == Player.X) myGlobal |= (1 << g);
                else oppGlobal |= (1 << g);
            } else if (ls == GameState.WON_O) {
                score += (perspective == Player.O ? 150 : -150) * w;
        ...
        // Global two-in-a-row threats (meta-game)
        score += globalThreatScore(myGlobal, oppGlobal, openMask) * 18;

        // Slight preference for having the opponent forced into a finished board
        // (gives us free choice next) - already partially in move ordering
    ...
    /** Count almost-complete lines on the global 3x3. */
    private int globalThreatScore(int my, int opp, int open) {
        ...
        for (int mask : WIN_MASKS) {
            ...
            if (theirs == 0) {
                if (mine == 2 && empties == 1) s += 12;   // one move from winning the game
                else if (mine == 1 && empties == 2) s += 3;
            }
            if (mine == 0) {
                if (theirs == 2 && empties == 1) s -= 12;
                else if (theirs == 1 && empties == 2) s -= 3;
            }
        }
        return s;
    }
    ...
    private int evaluateLocalThreats(Board board, int g, Player perspective) {
        ...
        for (int mask : WIN_MASKS) {
            ...
            if (my == 2 && emp == 1) score += 30;
            else if (my == 1 && emp == 2) score += 5;
            if (opp == 2 && emp == 1) score -= 30;
